...
float DecagonSoil = -99, DecagonSoil_e = -99, DecagonSoilTemp = -99;
float AcclimaSoil = -99, AcclimaTemp = -99, AcclimaPermittivity = -99, AcclimaConductivity = -99;
...
//	GPIO_SET_OUTPUT(GPIO_PORT_TO_BASE(GPIO_B_NUM), GPIO_PIN_MASK(5));
	...
	//Decagon Soil
	dec = DecagonSoil;
	frac = DecagonSoil - dec;	
	sprintf(StringBuffer, "%sDS=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Decagon Soil VWC
	dec = DecagonSoilTemp;
	frac = DecagonSoilTemp - dec;	
	sprintf(StringBuffer, "%sDT=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Decagon Soil Temp
	dec = AcclimaSoil;
	frac = AcclimaSoil - dec;	
	sprintf(StringBuffer, "%sAS=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Acclima Soil VWC
	dec = AcclimaTemp;
	frac = AcclimaTemp - dec;	
	sprintf(StringBuffer, "%sAT=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Acclima Soil Temp
	dec = AcclimaPermittivity;
	frac = AcclimaPermittivity - dec;	
	sprintf(StringBuffer, "%sAP=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Acclima Soil Permittivity
	dec = AcclimaConductivity;
	frac = AcclimaConductivity - dec;	
	sprintf(StringBuffer, "%sAC=%d.%02u,",StringBuffer, dec, (unsigned int) (frac*100));	//Add Acclima Soil Conductivity
...
static void askForDecagonMeasurement(void* ptr)
{
	printf("Ask for Decagon Measurement\r");
	uart_write_byte(1,'1');
	uart_write_byte(1,'D');
	uart_write_byte(1,'0');
	uart_write_byte(1,'!');
...
static void askForAcclimaMeasurement(void* ptr)
{
	printf("Ask for Acclima Measurement\r");
	uart_write_byte(1,'2');
	uart_write_byte(1,'D');
	uart_write_byte(1,'0');
	uart_write_byte(1,'!');
...
static void SDI12_line(void) {
	...
	if (strcmp(usart1_rx_buffer, "10012") == 0)
	{
		printf("Ask for Decagon Measurement in 1 second.\r");
		ctimer_set(&timer1, CLOCK_SECOND, askForDecagonMeasurement, NULL);
	}
	else if (strcmp(usart1_rx_buffer, "20014") == 0)
	{
		printf("Ask for Acclima Measurement in 2 seconds.\r");
		ctimer_set(&timer2, 2 * CLOCK_SECOND, askForAcclimaMeasurement, NULL);
	}
	else if (usart1_rx_buffer[0] == '1' && usart1_rx_buffer[1] == '+')
	{
		printf("Process Decagon reading\r");
		
		token = strtok(usart1_rx_buffer, "+");	
		while (token != NULL)
		{
			if (tokencount == 0) {
				//printf("TOKEN0:%s\r", token);
			}
			else if (tokencount == 1) {
				//printf("TOKEN1:%s\r", token);
				printf("Decagon Raw Permittivity: %s\r", token);
				DecagonSoil_e = atoi(token);
			}
			else if (tokencount == 2) {
				//printf("TOKEN2:%s\r", token);
				printf("Decagon Temp: %s\r", token);
				DecagonSoilTemp = atoi(token);
			}
			token = strtok(NULL, "+");
			tokencount++;
		}
				
		DecagonSoil_e = DecagonSoil_e / 50;	
		DecagonSoil = 4.3 * pow(10,-6) * pow(DecagonSoil_e,3) - 5.5 * pow(10,-4) * pow(DecagonSoil_e,2) + 2.92 * pow(10,-2) * DecagonSoil_e - 5.3 * pow(10,-2);
		DecagonSoil = DecagonSoil * 100;
		dec = DecagonSoil;
		frac = DecagonSoil - dec;	
		printf("Decagon Soil VWC: %d.%02u\r", dec, (unsigned int) (frac*100));		
	}
	else if (usart1_rx_buffer[0] == '2' && usart1_rx_buffer[1] == '+')
	{
		printf("Process Acclima reading\r");
		...
		while (token != NULL)
		{
			if (tokencount == 0) {
				...
				printf("Acclima VWC:%s\r", token);
				AcclimaSoil = atoi(token);
			}
			else if (tokencount == 2) {
				printf("Acclima Temp:%s\r", token);
				AcclimaTemp = atoi(token);
			}
			else if (tokencount == 3) {
				printf("Acclima Permittivity:%s\r", token);
				AcclimaPermittivity = atoi(token);
			}
			else if (tokencount == 4) {
				printf("Acclima Conductivity:%s\r", token);
				AcclimaConductivity = atoi(token);
			}
			token = strtok(NULL, "+");
			tokencount++;
		}
...
void TakeDecagonReading()
{
	printf("Take Decagon Reading\r");
	uart_write_byte(1,'1');
	uart_write_byte(1,'M');
	uart_write_byte(1,'!');
	uart_write_byte(1,'\r');
...
static void TakeAcclimaReading(void* ptr)
{
	printf("Take Acclima Reading\r");
	uart_write_byte(1,'2');
	uart_write_byte(1,'M');
	uart_write_byte(1,'!');
	uart_write_byte(1,'\r');
...
PROCESS_THREAD(example_mesh_process, ev, data)
{
  ...
  while(1) {
	...
	if(etimer_expired(&periodic)) {					//The send message event timer expired variable
		...
		//Turn on DecagonSoil Moisture Sensors (PORTB.5)
	//	GPIO_SET_OUTPUT(GPIO_PORT_TO_BASE(GPIO_B_NUM), GPIO_PIN_MASK(5));
