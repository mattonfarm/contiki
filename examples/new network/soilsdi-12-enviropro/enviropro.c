/* DecagonSoil Moisture Source Code for CC2538 version microcontroller
...
#define MEASUREMENTPERIOD_ADDR 0x18		//2bytes
...
char OneWireDeviceFound;											//True if we found one wire devices on the bus after a presence pulse
...
char MoistureString[100];	
char TemperatureString[100];	
char TemperatureMeasurement = false;
char MoistureMeasurement = false;
...
void send_message(void* ptr) {
	...
	int16_t value;						//Temporary variable for storing ADC readings. 32bit as readings are averaged
	...
	//Measure Regulator voltage (VCC / 3 (internal)) This is AVDD5 or the analog reference for all other ADC measurements
	...
	//Measure Analog 1
    value = adc_get(SOC_ADC_ADCCON_CH_AIN4, SOC_ADC_ADCCON_REF_AVDD5, SOC_ADC_ADCCON_DIV_512);
	A1 = value * (3300) / (2047 << 4);;

	//Measure Solar Voltage
    value = adc_get(SOC_ADC_ADCCON_CH_AIN5, SOC_ADC_ADCCON_REF_AVDD5, SOC_ADC_ADCCON_DIV_512);
	//A2 = (((A0 / 2047) * (value >> 4)) * 5.00) / 1000;
	//A2 = (((value / 20.0) * A0) / 4095);// * 11.95;
	A2 = (value * (3300) / (2047 << 4)) * 10.89;
	
	//Measure soil moisture battery voltage
	value = adc_get(SOC_ADC_ADCCON_CH_AIN7, SOC_ADC_ADCCON_REF_AVDD5, SOC_ADC_ADCCON_DIV_512);
	//Voltage = (((A0 / 2047) * (value >> 4)) * 5.00) / 1000;// * 4.091;
	Voltage = (value * (3300) / (2047 << 4)) * 4.88;
	...
	//Soil Moisture
	sprintf(StringBuffer, "%sSM=%s", StringBuffer,MoistureString);				//Add SM designator and print soil moisture values
	sprintf(StringBuffer, "%sST=%s", StringBuffer,TemperatureString);				//Add SM designator and print soil temperature values

	//Print the one wire temperature values. DecagonSoil moisture boards support 2 one wire sensors.
	for (i = 0; i < 2; i++)	{
...
//	sprintf(StringBuffer, "%s ~%d",StringBuffer, CRC);				//Add the CRC value to the string buffer
	...
	//Reset readings in case sensor goes offline
	sprintf(TemperatureString, " ");
	sprintf(MoistureString, " ");
...
static void process_line(void) {
	...
	else if (strncmp(usart_rx_buffer,"r",1) == 0)						//"r" in position 1 sets the measurement period in seconds. Minimum value = 10, Max is around 3000
	{
		if (usart_rx_buffer[4] >= 48 && usart_rx_buffer[4] <= 57)		//We are dealing with a 4 digit number
		{
			MeasurementPeriod = (usart_rx_buffer[1] - 48) * 1000;
			MeasurementPeriod += (usart_rx_buffer[2] - 48) * 100;
			MeasurementPeriod += (usart_rx_buffer[3] - 48) * 10;
			MeasurementPeriod += (usart_rx_buffer[4] - 48);
		}
		else if (usart_rx_buffer[3] >= 48 && usart_rx_buffer[3] <= 57)		//We are dealing with a 3 digit number
		{
...
void InitialiseEEPROMChipSelect(void)
{
...
void ReadFromEEPROM(void) {
	...
	SPI_FLUSH();
	SPI_READ(Temp);
	MeasurementPeriod = (Temp << 8);
	SPI_READ(Temp);
	MeasurementPeriod = MeasurementPeriod + (Temp & 0xff);
	watchdog_periodic();
	SPI_WAITFORTx_AFTER();
	SPI_CS_SET(GPIO_B_NUM, 1);
	clock_delay_usec(10);
...
void WriteToEEPROM(void)
{
	...
	temp = MeasurementPeriod >> 8;
	SPI_WRITE(temp);
	temp = MeasurementPeriod & 0xff;
	SPI_WRITE(temp);	
	SPI_CS_SET(GPIO_B_NUM, 1);									//Set CS high to start programming
	...
	watchdog_periodic();
	delay_msec(10);
	printf("Measurement Period = %d\r", MeasurementPeriod);
	watchdog_periodic();
...
void InitialiseOneWirePins(void)
{	
...
static void InitialiseDigitalInputPins(void)
{
	...
	GPIO_SET_PIN(GPIO_PORT_TO_BASE(GPIO_C_NUM), GPIO_PIN_MASK(5));					//Enable pull up
		
	/* Enable edge detection */
	GPIO_DETECT_EDGE(GPIO_PORT_TO_BASE(GPIO_C_NUM), GPIO_PIN_MASK(5));

	/* Single edge */
	GPIO_TRIGGER_SINGLE_EDGE(GPIO_PORT_TO_BASE(GPIO_C_NUM), GPIO_PIN_MASK(5));
	
	//Enable the interrupt
	GPIO_DETECT_RISING(GPIO_PORT_TO_BASE(GPIO_C_NUM), GPIO_PIN_MASK(5));
	GPIO_ENABLE_INTERRUPT(GPIO_PORT_TO_BASE(GPIO_C_NUM), GPIO_PIN_MASK(5));
		
}

static void InitialiseSDI12Module(void)
{
	//Set up module sleep pin
	ioc_set_over(GPIO_D_NUM, 0, IOC_OVERRIDE_DIS);
	GPIO_SET_OUTPUT(GPIO_PORT_TO_BASE(GPIO_D_NUM), GPIO_PIN_MASK(0));
	GPIO_CLR_PIN(GPIO_PORT_TO_BASE(GPIO_D_NUM), GPIO_PIN_MASK(0));	
	
	//Set up sensor power (Analog voltage port)
...
//Change the sleep status of the SDI12 module.
//Status 0 = sleep
//Status 1 = wake
//Also disable power saving when module is awake to keep serial comms
static void SDI12ConverterPower(char status)
{
	if (status == 0)
	{
		GPIO_CLR_PIN(GPIO_PORT_TO_BASE(GPIO_D_NUM), GPIO_PIN_MASK(0));
...
//Status 0 = sleep
//Status 1 = wake
static void EnviroProSensorPower(char status)
{
	if (status == 0)
		GPIO_CLR_PIN(GPIO_PORT_TO_BASE(GPIO_B_NUM), GPIO_PIN_MASK(5));
	else
...
static void askForMoistureMeasurement(void* ptr)
{
	printf("Ask for EnviroPro Moisture Measurement\r");
	uart_write_byte(1,'0');
	uart_write_byte(1,'D');
	uart_write_byte(1,'0');
	uart_write_byte(1,'!');
...
static void askForTemperatureMeasurement(void* ptr)
{
	printf("Ask for EnviroPro Temperature Measurement\r");
	uart_write_byte(1,'0');
	uart_write_byte(1,'D');
	uart_write_byte(1,'0');
	uart_write_byte(1,'!');
...
static void SDI12_line(void) {
	char* token;
	char tokencount = 0;
	
	if (strcmp(usart1_rx_buffer, "00014") == 0)
	{
		if (MoistureMeasurement == true)
		{
			
			printf("Ask for EnviroPro Moisture Measurement in .5 seconds.\r");
			ctimer_set(&timer1, 0.5 * CLOCK_SECOND, askForMoistureMeasurement, NULL);
		}
		else if (TemperatureMeasurement == true)
		{
			
			printf("Ask for EnviroPro Temperature Measurement in .5 seconds.\r");
			ctimer_set(&timer1, 0.5 * CLOCK_SECOND, askForTemperatureMeasurement, NULL);
		}
	}
	//If we have received a set of values back from address 0, do the following
	else if (usart1_rx_buffer[0] == '0' && usart1_rx_buffer[1] == '+')
	{
		//printf("Received 0+ Sequence\r");
		
		token = strtok(usart1_rx_buffer, "+");	
		
		if (TemperatureMeasurement == true)
		{	
			...
			while (token != NULL)
			{
				if (tokencount > 0)
				{
					strcat(TemperatureString, token);
		...
		{	
			...
			strcpy(MoistureString, "");
			while (token != NULL)
			{
				if (tokencount > 0)
				{
					strcat(MoistureString, token);
					strcat(MoistureString, ",");
				}				
				token = strtok(NULL, "+");
				tokencount++;
			}
		//	printf(MoistureString);
		//	printf("\r");
		}
		
		EnviroProSensorPower(0);
		SDI12ConverterPower(0);
		
		TemperatureMeasurement = false;
		MoistureMeasurement = false;
	}
...
int uart1_rx_callback(unsigned char c){
	uart_write_byte(0,c);
	if(usart1_rx_buffer_index < 499) {						//The usart buffer string is 500 chars long so this prevents an overflow
		if ((c >= 32 && c <= 128)){
	...
	if(c == '\r')											//If we rx a full carriage return terminated string, process the string
		SDI12_line();							
...
{
	EnviroProSensorPower(1);
	delay_msec(100);
	SDI12ConverterPower(1);
	delay_msec(100);
	printf("Take Moisture Reading\r");
	uart_write_byte(1,'0');
	uart_write_byte(1,'M');
	uart_write_byte(1,'!');
	uart_write_byte(1,'\r');
	uart_write_byte(1,'\n');
	TemperatureMeasurement = false;
	MoistureMeasurement = true;
	
}

static void TakeTemperatureReading(void* ptr)
{
	EnviroProSensorPower(1);
	delay_msec(100);
	SDI12ConverterPower(1);
	delay_msec(100);
	printf("Take Temperature Reading\r");
	uart_write_byte(1,'0');
	uart_write_byte(1,'M');
	uart_write_byte(1,'2');
	uart_write_byte(1,'!');
...
static void InitialiseADCPins(void)
{
...
static void
input_callback(uint8_t port, uint8_t pin)
{
      RainCount++;
      printf(" %d \n",RainCount);
...
PROCESS_THREAD(example_mesh_process, ev, data)
{
	...
	InitialiseSDI12Module();
	
	spi_init();											//Initialise the SPI bus (this library is included in Contiki and is cc2538 specific)
	InitialiseEEPROMChipSelect();
	ReadFromEEPROM();											//Read all our saved values from SPI EEPROM
		
	InitialiseOneWirePins();
	InitialiseADCPins();	
	InitialiseDigitalInputPins();
	
	//Register callback function for digital input pins
	gpio_register_callback(input_callback, GPIO_C_NUM, GPIO_PIN_MASK(5));
  ...
  PRINTF(" local/remote port %u/%u\n",
	...
	EnviroProSensorPower(1);
	delay_msec(200);
	printf("Scan for sensors - we use sensors at address 0\r");
	uart_write_byte(1,'0');
	uart_write_byte(1,'I');
	uart_write_byte(1,'!');
	uart_write_byte(1,'\r');
  ...
  while(1) {
	...
	if(etimer_expired(&periodic)) {					//The send message event timer expired variable
		...
		TakeMoistureReading();
		ctimer_set(&timer2, 2 * CLOCK_SECOND, TakeTemperatureReading, NULL);
